...
export default function ProfileScreen() {
  ...
  }, [editApiKey, profile, updateProfile]);
  ...
  return (
    <ScreenBackground>
      <View style={[styles.container, { paddingTop: insets.top }]}>
        ...
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
          <GlassCard style={styles.profileCard}>
            ...
            <Text style={styles.profileName}>{profile.name || 'LoveTestAI User'}</Text>
          ...
          {renderSettingsRow('key-outline', 'AI API Key', handleOpenApiKeyModal, profile.apiKey ? 'ACTIVE' : undefined)}
          ...
          {renderSettingsRow('star-outline', 'Rate LoveTestAI', handleRateApp)}
          ...
          <Text style={styles.footer}>
            LoveTestAI is for personal growth and entertainment. Not a licensed counselling service.
          </Text>
      ...
      <Modal visible={showApiKeyModal} transparent animationType="fade" onRequestClose={() => setShowApiKeyModal(false)}>
        <KeyboardAvoidingView style={styles.modalOverlay} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
          <View style={styles.modalContent}>
            <TouchableOpacity onPress={() => setShowApiKeyModal(false)} style={styles.modalClose}>
              <Ionicons name="close" size={24} color={colors.text_secondary} />
            </TouchableOpacity>
            <Text style={styles.modalTitle}>AI API Key</Text>
            <View>
              <Text style={styles.modalFieldLabel}>Anthropic API Key</Text>
              <TextInput
                style={styles.modalInput}
                value={editApiKey}
              ...
              <Text style={styles.apiKeyNote}>
                Enter your Anthropic API key to enable real AI responses. Without a key, mock responses will be used.
              </Text>
              <View style={styles.apiKeyStatus}>
                <Ionicons
                  name={editApiKey ? 'checkmark-circle' : 'ellipse-outline'}
                  size={16}
                  color={editApiKey ? colors.success : colors.text_muted}
                />
                <Text style={[editApiKey ? styles.apiKeyStatusText : styles.apiKeyStatusTextInactive]}>
                  {editApiKey ? 'Key configured' : 'No key set'}
                </Text>
              </View>
            </View>
            <View style={styles.modalActions}>
              <GradientButton label="Save Key" onPress={handleSaveApiKey} />
              {editApiKey ? (
                <GhostButton label="Remove Key" onPress={() => { setEditApiKey(''); }} />
              ) : null}
              <GhostButton label="Cancel" onPress={() => setShowApiKeyModal(false)} />
            </View>
          </View>
        </KeyboardAvoidingView>
      </Modal>
